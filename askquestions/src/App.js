...
export default function App() {
  ...
  useEffect(() => {
    ...
    listen("aiQuestions", v => setAiQuestions(v ? Object.values(v) : []));
  ...
  const startSolo = () => {
    ...
    setMode("solo"); setSoloQ(pickRandom(allQ, null));
    setSoloJustRated(false); setScreen("solo");
  };

  const soloNext = () => {
    setFlipping(true);
    setTimeout(() => { setSoloQ(pickRandom(allQ, soloQ?.id)); setSoloJustRated(false); setSoloStarHover(0); setFlipping(false); }, 280);
  };
  ...
  const nextQuestion = async () => {
    ...
    setFlipping(true);
    setTimeout(async () => {
      const q = pickRandom(allQ, gameState.currentQ?.id);
              ...
              {!adminAuthed ? (
                ...
                {adminTab === "bonus" && (
                  <div>
                    ...
                    <p className="ai-note">25 curated bonus questions in the spirit of Jeff's originals. These are always active and mixed into the deck automatically.</p>
                    <div className="hr" />
                    {BONUS_QUESTIONS.map(q => (
                      <div key={q.id} className="q-row">
                        <div style={{ flex: 1 }}>
                          ...
                          <p className="q-row-meta">{getAvgRating(q.id) ? `${getAvgRating(q.id)} ★ · ` : "unrated · "}Bonus</p>
                        </div>
                      </div>
                    ))}
                  </div>
  ...
  const allQ = [...JEFF_QUESTIONS, ...customQuestions, ...aiQuestions];
          ...
          <div className="overlay" onClick={e => e.target === e.currentTarget && setShowAdmin(false)}>
            <div className="modal">
              ...
              {!adminAuthed ? (
                ...
                <div className="tab-bar">
                  {[["instructions","How to Play"],["about","About"],["questions","Questions"],["ai","AI"],["stats","Insights"],["settings","Settings"]].map(([key, label]) => (
                    <button key={key} className={`tab ${adminTab === key ? "on" : ""}`} onClick={() => setAdminTab(key)}>{label}</button>
                  ))}
                </div>
                ...
                {adminTab === "ai" && (
                  <div>
                    <p className="sec-title">AI-Generated Questions</p>
                    <p className="ai-note">Claude generates 10 questions inspired by your highest-rated questions and steers away from patterns that get frequently skipped.</p>
                    <button className="btn-gold" style={{ width: "auto", marginBottom: "1rem" }} onClick={generateAiQ} disabled={aiLoading}>
                      {aiLoading ? "Generating…" : aiQuestions.length > 0 ? "↺ Regenerate 10 Questions" : "✦ Generate 10 Questions"}
                    </button>
                    {aiError && <p className="err">{aiError}</p>}
                    {aiQuestions.length > 0 && (<>
                      <div className="hr" />
                      <p className="ai-note">{aiQuestions.length} active AI questions</p>
                      {aiQuestions.map(q => (
                        <div key={q.id} className="q-row">
                          <div style={{ flex: 1 }}><p className="q-row-text">{q.text}</p>{q.hint && <p className="q-row-meta">💡 {q.hint}</p>}<p className="q-row-meta">{getAvgRating(q.id) ? `${getAvgRating(q.id)} ★ · ` : "unrated · "}Claude</p></div>
                          <button className="icon-btn del" onClick={() => deleteAiQ(q.id)}>✕</button>
                        </div>
                      ))}
                    </>)}
                  </div>
